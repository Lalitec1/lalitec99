...
*/
int funciondenteros(int intB){ /*prototipo de funcion*/
	int intByteL=0x000000ff&intB;
	int intByteH=0x000000ff&(intB>>8);
	int tmp=intByteL;
	intByteL=intByteH;
	intByteH=tmp;
...
//	intWordL=intByteL|(tmp<<8);
	intWordH=funciondenteros(intBH);
	...
	intByteL=intByteH;
	intWordL=intByteL|(tmp<<8);*/
	intWordL=funciondenteros(intBL);
	//printf("intWordH=0x%x\n",intWordH);
	p=&ache;
	/*p->saludo=0x00414c4f;*/
	p->saludo=tmp=(0xffff0000&(intWordL<<16))|intWordH;
